# by @knight_errent
# Now will be used in jackzbot too....
import asyncio
import datetime
...
async def set_not_afk(event):
    ...
    if ".afk" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        jackzbot = await borg.send_message(
            event.chat_id,
            "🔥__WAPIS ZINDA HO GYA!__\n**No Longer afk.**\n⏱️ `Was afk for:``"
            + total_afk_time
            + "`",
            file=jackzpic,
        )
        try:
            ...
            await borg.send_message(  # pylint:disable=E0602
                ...
                + "Ask in @jackzbot_official_Chat to get help setting this value\n\n `{}`".format(
                    str(e)
                ),
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:
        ...
        msg = await event.reply(message_to_reply, file=jackzpic)
...
async def _(event):
    ...
    jackzpic = await event.client.download_media(krakenop)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        USER_AFK = f"yes: {reason} {jackzpic}"  # pylint:disable=E0602
        if reason:
            await borg.send_message(
                event.chat_id,
                f"__**BIE BIE...TATA🚶**__ \n⚜️ Because `{reason}`",
                file=jackzpic,
            )
        else:
            await borg.send_message(
                event.chat_id, f"**BIE BIE...TATA!**🚶", file=jackzpic
            )
